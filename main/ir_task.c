...
static const char *TAG = "IR_RX_task";
...
static void IRAM_ATTR ir_rx_intr_handler(void *arg)
{
    ...
    case IR_RX_IDLE:
    {
        if (time_escape < IR_RX_DATA1_US + IR_RX_ERROR_US && time_escape > IR_RX_DATA0_US - IR_RX_ERROR_US)
            ir_state = IR_RX_DATA;
    }
    break;
    ...
    case IR_RX_DATA:
    {
        if (time_escape < IR_RX_DATA1_US + IR_RX_ERROR_US && time_escape > IR_RX_DATA1_US - IR_RX_ERROR_US)
            tmp |= (0x1 << (IR_RX_BIT_NUM - 1 - cnt++));
        else if (time_escape < IR_RX_DATA0_US + IR_RX_ERROR_US && time_escape > IR_RX_DATA0_US - IR_RX_ERROR_US)
            tmp |= (0x0 << (IR_RX_BIT_NUM - 1 - cnt++));
        else if (time_escape > IR_RX_DATA1_US + IR_RX_ERROR_US)
            goto reset_status;

        if (cnt == IR_RX_BIT_NUM)
        {
            // push rcv data to ringbuf
            ir_data.byte = tmp;
            xRingbufferSendFromISR(ir_rx_obj->ring_buf, (void *)&ir_data, sizeof(ir_rx_data_t) * 1, &xHigherPriorityTaskWoken);
            goto reset_status;
        }
    }
    break;
...
int ir_rx_recv_data(ir_rx_data_t *data, size_t len, uint32_t timeout_ticks)
{
    ...
    for (int x = 0; x < len;)
    {
        buf = (ir_rx_data_t *)xRingbufferReceive(ir_rx_obj->ring_buf, &size, timeout_ticks);
...
esp_err_t ir_rx_init(ir_rx_config_t *config)
{
    ...
    ir_rx_obj->ring_buf = xRingbufferCreate(sizeof(ir_rx_data_t) * ir_rx_obj->buf_len, RINGBUF_TYPE_NOSPLIT);
